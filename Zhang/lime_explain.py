import multiple_parallel_convolutional_layers_net as mpc
import numpy as np
...
with tf.device('/GPU:0'):
    net, X, Y, X_eval, Y_eval = mpc.train_speck_distinguisher(num_epochs=40, num_rounds=5, depth=5, pairs=16)
...
sp_obj = SubmodularPick(explainer, X_eval, predict_fn, num_features=X_eval.shape[1], num_exps_desired=200,sample_size=500,method='sample')
explanations = sp_obj.sp_explanations
i = 1
#Writing explanation data to csv file
with open('lime_explanation_200_samples_6round_2pairs_2blocks.csv', mode='w', newline='') as file:
